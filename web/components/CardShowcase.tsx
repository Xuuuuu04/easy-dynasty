...
export default function CardShowcase({ card, onComplete }: CardShowcaseProps) {
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center" onClick={handleSkip}>
            ...
            <div
                className="relative w-[280px] h-[480px] md:w-[320px] md:h-[560px] perspective-1000 z-10"
            >
                <div
                    className={`
          ...
          `}
                >
                    ...
                    <div
                        className="absolute inset-0 backface-hidden rotate-y-180 rounded-xl overflow-hidden shadow-2xl border-4 border-[#9a2b2b]"
                    >
                        <div className="relative w-full h-full bg-white">
                            ...
                            <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/90 via-black/40 to-transparent p-6 pt-12">
                                <p className="text-white font-serif font-bold text-2xl text-center tracking-widest">{card.name}</p>
                                <p className="text-white/70 text-xs text-center uppercase tracking-widest mt-1">{card.englishName}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Back (Card Back) */}
                    <div
                        className="absolute inset-0 backface-hidden rounded-xl overflow-hidden shadow-2xl border-2 border-[#4a4a4a]"
                    >
                        <Image
                            src={CARD_BACK_IMAGE}
                            alt="Card Back"
            ...
            {/* Skip Hint */}
            <div className={`absolute bottom-20 text-white/50 text-sm tracking-widest transition-opacity duration-300 ${phase === 'exit' ? 'opacity-0' : 'opacity-100'}`}>
                点击屏幕跳过
            </div>
