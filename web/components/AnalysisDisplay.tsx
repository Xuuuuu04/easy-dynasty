...
export default function AnalysisDisplay({
  ...
  return (
    <div className="ink-card p-6 md:p-8 flex flex-col animate-slide-up" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="prose prose-stone max-w-none">
            <ReactMarkdown
              ...
              components={{
                ...
                table: ({ children }) => <div className="overflow-x-auto my-8 border border-stone-200 rounded-sm shadow-sm"><table className="min-w-full divide-y divide-stone-200">{children}</table></div>,
                thead: ({ children }) => <thead className="bg-stone-50">{children}</thead>,
                th: ({ children }) => <th className="px-4 py-3 text-left text-xs font-bold text-stone-500 uppercase tracking-wider border-r border-stone-200 last:border-r-0">{children}</th>,
                td: ({ children }) => <td className="px-4 py-3 text-sm text-stone-600 border-r border-stone-100 last:border-r-0">{children}</td>,
