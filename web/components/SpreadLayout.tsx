    ...
    return (
      ...
      >
        ...
        <div className="absolute top-0 left-1/2 z-10 -translate-x-1/2">
          <div className="inline-flex items-center gap-1.5 rounded-sm border border-[#9a2b2b] bg-white/95 px-3 py-1 text-xs font-bold uppercase tracking-wider text-[#9a2b2b] shadow-sm whitespace-nowrap transition-all group-hover:scale-105">
        ...
        {/* Card Slot */}
        <div
          className={`relative flex w-32 items-center justify-center rounded-lg border border-dashed transition-all duration-500 ${canDraw
              ? 'cursor-pointer border-[#9a2b2b]/40 bg-[#9a2b2b]/5 hover:border-[#9a2b2b] hover:shadow-md hover:bg-[#9a2b2b]/10'
              : 'border-stone-300 bg-stone-100/50'
            } ${isCurrentlyDrawing
              ? 'border-[#9a2b2b] bg-[#9a2b2b]/10 shadow-md animate-pulse'
              : ''
            }`}
        ...
        >
          {drawnCard ? (
            ...
            <div className="relative flex flex-col items-center gap-2 text-center text-sm z-10 px-2">
              {isCurrentlyDrawing ? (
                <span className="text-[#9a2b2b] font-bold animate-pulse">Drawing...</span>
              ) : canDraw ? (
                <span className="text-[#9a2b2b]/80 font-medium group-hover:text-[#9a2b2b] transition-colors">Click to Draw</span>
              ) : (
                <span className="text-stone-400 text-xs uppercase tracking-widest">Waiting</span>
              )}
              <span className="text-[10px] leading-relaxed text-stone-500 group-hover:text-stone-600 transition-colors line-clamp-3">
        ...
        {drawnCard && (
          <div className="mt-4 w-36 text-center animate-fade-in">
            <div className="text-sm font-bold text-ink mb-0.5 font-serif">
              {drawnCard.card.name}
            </div>
        ...
        {!drawnCard && (
          <div className="mt-4 w-32 text-center text-[10px] leading-relaxed text-stone-400 group-hover:text-stone-500 transition-colors">
            {position.description}
          </div>
        )}
      </div>
      ...
      return (
        <div className="min-h-[900px] max-w-7xl mx-auto px-4 py-8 overflow-x-auto">
          <div className="min-w-[800px] grid grid-cols-7 gap-8 max-w-6xl mx-auto">
            ...
            {/* Right Staff */}
            <div className="col-span-2 flex flex-col justify-center gap-6 pl-8 border-l border-stone-300">
